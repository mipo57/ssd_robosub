...
class Augmenter:
    ...
    def augment_image(self, image: np.ndarray, bounding_box: BoundingBox, label_is_present: bool):
        ...
        aug_p = label_is_present and len(aug_bbs.bounding_boxes) > 0 #= label_is_present and aug_bbs.shape[0] > 0

        aug_coords = BoundingBox.from_points(0,0,0,0)
        if aug_p:
            aug_coords = aug_bbs.bounding_boxes[0]

        return aug_image, \
               BoundingBox.from_points(x1=aug_coords.x1,
                               y1=aug_coords.y1,
                               x2=aug_coords.x2,
                               y2=aug_coords.y2), \
               aug_p
...
class NoAgumenter:
    ...
    def augment_image(self, image: np.ndarray, bounding_box: BoundingBox, label_is_present: bool):
        return image, bounding_box, label_is_present
